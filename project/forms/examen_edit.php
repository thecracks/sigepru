...
require(__PANEL__ . '/ExamenEditPanel.class.php');

/**
 * This is a draft QForm object to do Create, Edit, and Delete functionality
 * of the Examen class.  It uses the code-generated
 * ExamenConnector class, which has methods to help with
 * easily creating/defining controls to modify the fields of Examen columns.
 *
 ...
 * @subpackage Drafts
 */
class ExamenEditForm extends QForm {
...
//	protected function Form_Load() {}
	...
	protected function Form_Create() {
        ...
        $this->strObjectName = QApplication::Translate('Examen');
        $this->strObjectNamePlural = QApplication::Translate('Examens');

        $this->pnlExamen = new ExamenEditPanel($this);
		$strExamenId = QApplication::QueryString('strExamenId');
	    $this->pnlExamen->Load($strExamenId);
		$this->CreateButtons();
	}
	...
	protected function CreateButtons() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('Examen'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->pnlExamen->mctExamen->EditMode;
	}
    ...
    */
    protected function dlgOptimisticLocking_ButtonEvent($strFormId, $strControlId, $btn) {
        if ($btn == "Overwrite") {
            $this->pnlExamen->Save(true);
            $this->GetControl($strControlId)->Close();
            $this->RedirectToListPage();
        } else { // Refresh
            $this->GetControl($strControlId)->Close();
            $this->pnlExamen->Refresh(true);
        }
    }
    ...
    */
	protected function RedirectToListPage() {
		QApplication::Redirect(__VIRTUAL_DIRECTORY__ . __FORMS__ . '/examen_list.php',
...
// examen_edit.tpl.php as the included HTML template file
ExamenEditForm::Run('ExamenEditForm');
