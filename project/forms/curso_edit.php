...
require(__PANEL__ . '/CursoEditPanel.class.php');

/**
 * This is a draft QForm object to do Create, Edit, and Delete functionality
 * of the Curso class.  It uses the code-generated
 * CursoConnector class, which has methods to help with
 * easily creating/defining controls to modify the fields of Curso columns.
 *
 ...
 * @subpackage Drafts
 */
class CursoEditForm extends QForm {
...
//	protected function Form_Load() {}
	...
	protected function Form_Create() {
        ...
        $this->strObjectName = QApplication::Translate('Curso');
        $this->strObjectNamePlural = QApplication::Translate('Cursos');

        $this->pnlCurso = new CursoEditPanel($this);
		$strCursoId = QApplication::QueryString('strCursoId');
	    $this->pnlCurso->Load($strCursoId);
		$this->CreateButtons();
	}
	...
	protected function CreateButtons() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('Curso'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->pnlCurso->mctCurso->EditMode;
	}
    ...
    */
    protected function dlgOptimisticLocking_ButtonEvent($strFormId, $strControlId, $btn) {
        if ($btn == "Overwrite") {
            $this->pnlCurso->Save(true);
            $this->GetControl($strControlId)->Close();
            $this->RedirectToListPage();
        } else { // Refresh
            $this->GetControl($strControlId)->Close();
            $this->pnlCurso->Refresh(true);
        }
    }
    ...
    */
	protected function RedirectToListPage() {
		QApplication::Redirect(__VIRTUAL_DIRECTORY__ . __FORMS__ . '/curso_list.php',
...
// curso_edit.tpl.php as the included HTML template file
CursoEditForm::Run('CursoEditForm');
