...
require(__PANEL__ . '/RestaurantEditPanel.class.php');
 ...
 * RestaurantConnector class, which has methods to help with
 * easily creating/defining controls to modify the fields of Restaurant columns.
 *
 ...
 * @subpackage Drafts
 */
class RestaurantEditForm extends QForm {

	/** @var RestaurantEditPanel  */
	protected $pnlRestaurant;
...
//	protected function Form_Load() {}
	...
	protected function Form_Create() {
        ...
        $this->strObjectName = QApplication::Translate('Restaurant');
        $this->strObjectNamePlural = QApplication::Translate('Restaurants');

        $this->pnlRestaurant = new RestaurantEditPanel($this);
		$intIdRestaurant = QApplication::QueryString('intIdRestaurant');
	    $this->pnlRestaurant->Load($intIdRestaurant);
		$this->CreateButtons();
	}
	...
	protected function CreateButtons() {
		...
		$this->btnDelete->Visible = $this->pnlRestaurant->mctRestaurant->EditMode;
	}
    ...
    */
    protected function dlgOptimisticLocking_ButtonEvent($strFormId, $strControlId, $btn) {
        if ($btn == "Overwrite") {
            $this->pnlRestaurant->Save(true);
            $this->GetControl($strControlId)->Close();
            $this->RedirectToListPage();
        } else { // Refresh
            $this->GetControl($strControlId)->Close();
            $this->pnlRestaurant->Refresh(true);
        }
    }
    ...
    */
	protected function RedirectToListPage() {
		QApplication::Redirect(__VIRTUAL_DIRECTORY__ . __FORMS__ . '/restaurant_list.php',
...
// restaurant_edit.tpl.php as the included HTML template file
RestaurantEditForm::Run('RestaurantEditForm');
