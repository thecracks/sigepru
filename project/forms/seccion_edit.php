...
require(__PANEL__ . '/SeccionEditPanel.class.php');

/**
 * This is a draft QForm object to do Create, Edit, and Delete functionality
 * of the Seccion class.  It uses the code-generated
 * SeccionConnector class, which has methods to help with
 * easily creating/defining controls to modify the fields of Seccion columns.
 *
 ...
 * @subpackage Drafts
 */
class SeccionEditForm extends QForm {

	/** @var SeccionEditPanel  */
	protected $pnlSeccion;
...
//	protected function Form_Load() {}
	...
	protected function Form_Create() {
        ...
        $this->strObjectName = QApplication::Translate('Seccion');
        $this->strObjectNamePlural = QApplication::Translate('Seccions');

        $this->pnlSeccion = new SeccionEditPanel($this);
		$strSeccionId = QApplication::QueryString('strSeccionId');
	    $this->pnlSeccion->Load($strSeccionId);
		$this->CreateButtons();
	}
	...
	protected function CreateButtons() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('Seccion'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->pnlSeccion->mctSeccion->EditMode;
	}
    ...
    */
    protected function dlgOptimisticLocking_ButtonEvent($strFormId, $strControlId, $btn) {
        if ($btn == "Overwrite") {
            $this->pnlSeccion->Save(true);
            $this->GetControl($strControlId)->Close();
            $this->RedirectToListPage();
        } else { // Refresh
            $this->GetControl($strControlId)->Close();
            $this->pnlSeccion->Refresh(true);
        }
    }
    ...
    */
	protected function RedirectToListPage() {
		QApplication::Redirect(__VIRTUAL_DIRECTORY__ . __FORMS__ . '/seccion_list.php',
...
// seccion_edit.tpl.php as the included HTML template file
SeccionEditForm::Run('SeccionEditForm');
