...
require(__PANEL__ . '/DocenteEditPanel.class.php');

/**
 * This is a draft QForm object to do Create, Edit, and Delete functionality
 * of the Docente class.  It uses the code-generated
 * DocenteConnector class, which has methods to help with
 * easily creating/defining controls to modify the fields of Docente columns.
 *
 ...
 * @subpackage Drafts
 */
class DocenteEditForm extends QForm {

	/** @var DocenteEditPanel  */
	protected $pnlDocente;
...
//	protected function Form_Load() {}
	...
	protected function Form_Create() {
        ...
        $this->pnlDocente = new DocenteEditPanel($this);
		$strDocenteId = QApplication::QueryString('strDocenteId');
	    $this->pnlDocente->Load($strDocenteId);
		$this->CreateButtons();
	}
	...
	protected function CreateButtons() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('Docente'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->pnlDocente->mctDocente->EditMode;
	}
    ...
    */
    protected function dlgOptimisticLocking_ButtonEvent($strFormId, $strControlId, $btn) {
        if ($btn == "Overwrite") {
            $this->pnlDocente->Save(true);
            $this->GetControl($strControlId)->Close();
            $this->RedirectToListPage();
        } else { // Refresh
            $this->GetControl($strControlId)->Close();
            $this->pnlDocente->Refresh(true);
        }
    }
    ...
    */
	protected function RedirectToListPage() {
		QApplication::Redirect(__VIRTUAL_DIRECTORY__ . __FORMS__ . '/docente_list.php',
...
// docente_edit.tpl.php as the included HTML template file
DocenteEditForm::Run('DocenteEditForm');
