...
require(__PANEL__ . '/AlumnoEditPanel.class.php');

/**
 * This is a draft QForm object to do Create, Edit, and Delete functionality
 * of the Alumno class.  It uses the code-generated
 * AlumnoConnector class, which has methods to help with
 * easily creating/defining controls to modify the fields of Alumno columns.
 *
 ...
 * @subpackage Drafts
 */
class AlumnoEditForm extends QForm {
...
//	protected function Form_Load() {}
	...
	protected function Form_Create() {
        ...
        $this->strObjectName = QApplication::Translate('Alumno');
        $this->strObjectNamePlural = QApplication::Translate('Alumnos');

        $this->pnlAlumno = new AlumnoEditPanel($this);
		$strAlumnoId = QApplication::QueryString('strAlumnoId');
	    $this->pnlAlumno->Load($strAlumnoId);
		$this->CreateButtons();
	}
	...
	protected function CreateButtons() {
		...
		$this->btnDelete->AddAction(new QClickEvent(), new QConfirmAction(sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'), QApplication::Translate('Alumno'))));
		$this->btnDelete->AddAction(new QClickEvent(), new QAjaxAction('btnDelete_Click'));
		$this->btnDelete->Visible = $this->pnlAlumno->mctAlumno->EditMode;
	}
    ...
    */
    protected function dlgOptimisticLocking_ButtonEvent($strFormId, $strControlId, $btn) {
        if ($btn == "Overwrite") {
            $this->pnlAlumno->Save(true);
            $this->GetControl($strControlId)->Close();
            $this->RedirectToListPage();
        } else { // Refresh
            $this->GetControl($strControlId)->Close();
            $this->pnlAlumno->Refresh(true);
        }
    }
    ...
    */
	protected function RedirectToListPage() {
		QApplication::Redirect(__VIRTUAL_DIRECTORY__ . __FORMS__ . '/alumno_list.php',
...
// alumno_edit.tpl.php as the included HTML template file
AlumnoEditForm::Run('AlumnoEditForm');
