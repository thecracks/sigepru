...
/**
 * This is the base list class for the  Balance class.
 *
 ...
 */
abstract class BalanceListPanelGen extends QPanel {
	/** @var QPanel **/
	protected $pnlFilter;
	...
	public function __construct($objParent, $strControlId = null) {
		...
		$this->dtgBalances_Create();
		$this->dtgBalances->SetDataBinder('BindData', $this);
	...
	**/
	protected function dtgBalances_Create() {
		$this->dtgBalances = new BalanceList($this);
		$this->dtgBalances_CreateColumns();
		$this->dtgBalances_MakeEditable();
		$this->dtgBalances->RowParamsCallback = [$this, "dtgBalances_GetRowParams"];
			$this->dtgBalances->LinkedNode = QQN::Balance();
	}
	...
	**/
	protected function dtgBalances_CreateColumns() {
		$this->dtgBalances->CreateColumns();
	}

	protected function dtgBalances_MakeEditable() {
		$this->dtgBalances->AddAction(new QCellClickEvent(0, null, null, true), new QAjaxControlAction($this, 'dtgBalances_CellClick', null, null, QCellClickEvent::RowValue));
		$this->dtgBalances->AddCssClass('clickable-rows');
	}

	protected function dtgBalances_CellClick($strFormId, $strControlId, $strParameter) {
		if ($strParameter) {
			$this->EditItem($strParameter);
		}
	}
	public function dtgBalances_GetRowParams($objRowObject, $intRowIndex) {
	...
	public function BindData() {
		...
		$this->dtgBalances->BindData($objCondition);
	}
	...
	protected function GetCondition() {
		...
		if (is_null($strSearchValue) || $strSearchValue === '') {
			...
			return QQ::OrCondition(
				QQ::Equal(QQN::Balance()->IdBalance, $strSearchValue),
            QQ::Like(QQN::Balance()->AmountExchangedCoins, "%" . $strSearchValue . "%"),
            QQ::Equal(QQN::Balance()->IdUser, $strSearchValue),
            QQ::Equal(QQN::Balance()->IdOffer, $strSearchValue)
			);
		}
	...
	protected function EditItem ($strKey = null) {
		$strQuery = '';
		if ($strKey) {
			$strQuery =  '?intIdBalance=' . $strKey;
		}
		$strEditPageUrl = __VIRTUAL_DIRECTORY__ . __FORMS__ . '/balance_edit.php' . $strQuery;
