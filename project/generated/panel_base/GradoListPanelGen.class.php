...
/**
 * This is the base list class for the  Grado class.
 *
 ...
 */
abstract class GradoListPanelGen extends QPanel {
	/** @var QPanel **/
	protected $pnlFilter;
	...
	public function __construct($objParent, $strControlId = null) {
		...
		$this->dtgGrados_Create();
		$this->dtgGrados->SetDataBinder('BindData', $this);
	...
	**/
	protected function dtgGrados_Create() {
		$this->dtgGrados = new GradoList($this);
		$this->dtgGrados_CreateColumns();
		$this->dtgGrados_MakeEditable();
		$this->dtgGrados->RowParamsCallback = [$this, "dtgGrados_GetRowParams"];
			$this->dtgGrados->LinkedNode = QQN::Grado();
	}
	...
	**/
	protected function dtgGrados_CreateColumns() {
		$this->dtgGrados->CreateColumns();
	}

	protected function dtgGrados_MakeEditable() {
		$this->dtgGrados->AddAction(new QCellClickEvent(0, null, null, true), new QAjaxControlAction($this, 'dtgGrados_CellClick', null, null, QCellClickEvent::RowValue));
		$this->dtgGrados->AddCssClass('clickable-rows');
	}

	protected function dtgGrados_CellClick($strFormId, $strControlId, $strParameter) {
		if ($strParameter) {
			$this->EditItem($strParameter);
		}
	}
	public function dtgGrados_GetRowParams($objRowObject, $intRowIndex) {
	...
	public function BindData() {
		...
		$this->dtgGrados->BindData($objCondition);
	}
	...
	protected function GetCondition() {
		...
		if (is_null($strSearchValue) || $strSearchValue === '') {
			...
			return QQ::OrCondition(
				QQ::Like(QQN::Grado()->GradoId, "%" . $strSearchValue . "%"),
            QQ::Like(QQN::Grado()->Grado, "%" . $strSearchValue . "%"),
            QQ::Like(QQN::Grado()->Createby, "%" . $strSearchValue . "%"),
            QQ::Like(QQN::Grado()->Updateby, "%" . $strSearchValue . "%"),
            QQ::Like(QQN::Grado()->Active, "%" . $strSearchValue . "%"),
            QQ::Like(QQN::Grado()->NivelId, "%" . $strSearchValue . "%")
			);
		}
	...
	protected function EditItem ($strKey = null) {
		$strQuery = '';
		if ($strKey) {
			$strQuery =  '?strGradoId=' . $strKey;
		}
		$strEditPageUrl = __VIRTUAL_DIRECTORY__ . __FORMS__ . '/grado_edit.php' . $strQuery;
