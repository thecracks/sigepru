...
include (__PANEL__ . '/BalanceEditPanel.class.php');

/**
 * This is the BalanceEditDlgGen class.  It uses the code-generated
 * BalanceEditPanel class, which has all the controls for editing
 * a record in the balance table.
 *
 *
 * @package My QCubed Application
 * @subpackage Dialogs
 */
class BalanceEditDlgGen extends QDialog {

	/** @var BalanceEditPanel  */
	protected $pnlBalance;
	...
	public function __construct($objParentObject = null, $strControlId = null) {
		...
		$this->pnlBalance = new BalanceEditPanel($this);
		$this->CreateButtons();
	}
	...
	protected function CreateButtons() {
		...
		$this->AddButton(QApplication::Translate('Delete'), 'delete', false, false,
			sprintf(QApplication::Translate('Are you SURE you want to DELETE this %s?'),  QApplication::Translate('Balance')),
	...
	public function Load ($intIdBalance = null) {
		$this->pnlBalance->Load($intIdBalance);
		$blnIsNew = is_null($intIdBalance);
    ...
    */
    protected function dlgOptimisticLocking_ButtonEvent($strFormId, $strControlId, $btn) {
        if ($btn == "Overwrite") {
            $this->pnlBalance->Save(true);
            $this->Form->GetControl($strControlId)->Close();
            $this->Close();
        } else { // Refresh
            $this->Form->GetControl($strControlId)->Close();
            $this->pnlBalance->Refresh(true);
        }
    }
